...
#define NULINK_READ_TIMEOUT  (6000)
...
static int nulink_usb_open(struct hl_interface_param_s *param, void **fd)
{
	...
	const uint16_t vids[] = { 0x0416, 0x0416, 0x0416, 0x0416, 0x0416, 0x0416, 0x0416, 0x0416, 0x0416, 0x0416, 0x0416, 0x0416, param->vid, 0 };
	const uint16_t pids[] = { 0x5200, 0x5201, 0x5202, 0x5203, 0x5204, 0x5205, 0x2004, 0x2005, 0x2006, 0x2007, 0x2008, 0x511c, param->pid, 0 };
	...
	while (cur_dev) {
		...
		for (unsigned int i = 0; vids[i] || pids[i]; ++i) {
